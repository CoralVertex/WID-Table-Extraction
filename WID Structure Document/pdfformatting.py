...
fieldDetails = pd.DataFrame(columns = ['TableName','OrigFieldDetail','FieldName','FieldDesc','TitleLength'])

# create field details table
for tname in table['TableName']:
    listM = table.index[table['TableName'] == tname].tolist()
    m = listM[0]
    rows = len(pd.Series(table[table['TableName'] == tname]['FieldDetails'][m]).index)
    a = 0
    while a < rows:
        if not re.search(' ',table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ','')):
            fieldDetails = fieldDetails.append({'TableName':tname, 'OrigFieldDetail':table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ',''), 'FieldName':table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ',''), 'FieldDesc':table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ','')}, ignore_index=True)
        elif re.search("\) ",table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ','')):
            fieldDetails = fieldDetails.append({'TableName':tname, 'OrigFieldDetail':table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ',''), 'FieldName':table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ','').split(' ')[1], 'FieldDesc':table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ','').split(") ")[1]}, ignore_index=True)
        elif re.search('date ',table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ','')):
            fieldDetails = fieldDetails.append({'TableName':tname, 'OrigFieldDetail':table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ',''), 'FieldName':table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ','').split(' ')[1], 'FieldDesc':table[table['TableName'] == tname]['FieldDetails'][m][a].replace('Primary Key ','').split('date ')[1]}, ignore_index=True)
        
        a = a + 1

x = 0
while x < len(fieldDetails.index): 
    fieldDetails['TitleLength'][x]=len(fieldDetails['OrigFieldDetail'][x])
    x = x + 1

display(fieldDetails)


fieldDetails.to_excel("fieldDetails.xlsx")
table.to_excel("tableDescriptions.xlsx")
